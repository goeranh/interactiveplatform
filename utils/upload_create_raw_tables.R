...
# Update the .rdata[['full_indicators']] variable with the indicators in category
update_full_indicators <- function(choosen_category = .rdata[['categories_indicators']]$category[1]){
  
  indicator_codes <- select(filter(.rdata[['categories_indicators']], category == choosen_category), code)
...
fill_in_global_list <- function(){
  ...
  first_category <- .rdata[['categories_indicators']]$category[1]
  
  # to always show all AVAILABLE indicators for selected category
  indicator_codes <- select(filter(.rdata[['categories_indicators']], category == first_category), code)
  ...
  focus <- filter(.rdata[['strata']], (country == focus_country & indic  %in% indicator_codes$code)) %>% 
    arrange(desc(year), indic_name, dimension)
  
  .rdata[['all_countries']] <<- countries
  .rdata[['focus_category']] <<- first_category
  ...
  # Updates values in .rdata[['equity_dimensions']]
  update_global_dimensions(focus$indic[1])  
