...
# .rdata[['summary_measures_all']] <- subset(summeasures$summeasurevect, summeasures$summeasurevect == 'd' | summeasures$summeasurevect == 'r')
.rdata[['summary_measures_table']] <- summeasures$summeasure
.rdata[['inequal_rules_tmp']] <- summeasures$summeasure_just2_ordered_se


# *****************************************************************************
# Read categories and indicators associations

# *****************************************************************************
.rdata[['categories_indicators']] <- readxl::read_xlsx("data/categories_indicators.xlsx", sheet=1)

.rdata[['full_categories']] <- unique(select(.rdata[['categories_indicators']], category))
...
.rdata[['indicators_metadata']] <- readxl::read_xlsx("data/indicators_metadata.xlsx", sheet=1)

.rdata[['indicators_categories']] <- unique(select(readxl::read_xlsx("data/indicators_metadata.xlsx", sheet=2), 
                                                   Categories))
