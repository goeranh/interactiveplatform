...
getInequalData <- function(indicator = NULL, stratifier = NULL, countries = NULL, years = NULL,
  ...
  if(!is.null(mostrecent) && mostrecent){
    ...
    # my fix all nations
    if(!is.null(anchorCountry)) {
      
      years <- getFilteredYear(country=anchorCountry, datasource)[1]
    } else {
      
      years = 0
      
      for(item in countries) {
        countryYear = getFilteredYear(country=item, datasource)[1]
        if(countryYear > years) years = countryYear
      }
    }
  }
