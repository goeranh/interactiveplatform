#15/05/2019: update per autofocus su asse y --> r 11
plotDisagBar_explore_hc <- function(plotData, confInt = FALSE, ...) {
  ...
  axismin <- if (axismin == "") min(0, floor(plotData$estimate)) else as.numeric(axismin)
  
  axismax <- isolate(input$axis_limitsmax1)
  axismax <- if (axismax == "") max(plotData$estimate + 2, na.rm = TRUE) else axismax
