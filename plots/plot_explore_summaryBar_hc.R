#23/05/2019: update for negative bar - line 8
plotSummaryBar_explore_hc <- function(plotData, confInt = FALSE, ...) {
  ...
  #axismin <- if (axismin == "") 0 else as.numeric(axismin)
  axismin <- ifelse(axismin != "", as.numeric(axismin),ifelse(min(plotData$inequal)<0,min(plotData$inequal)-1,0))
