...
output$zoomhc_compare <- renderHighchart({
  ...
  uselegend <- TRUE #abilita o disabilita la legenda nel grafico zoomato
  ...
  if(input$comparison_panel == "inequaldisag") {
    ...
    regressionData <- .rdata[['focus_plot_regression_summary_compare']]
    hc <- fun_hc(focus_data, regressionData, nchart = floor(input$nchart))
  }
  
  #git1115
  # if (whichPlot == "plotSummaryBar_compare_hc" && isolate(input$disag_plot_summary_pts) == "labels") {
...
getGrid <- function(plotDataChart, title = "This is the main title", legend = TRUE,
  ...
  if(!is.null(minY) && minY != "") {
    plotDataChart$chart <- map(plotDataChart$chart, hc_yAxis, min = minY, endOnTick = FALSE, startOnTick = FALSE)
  }
  
  if(!is.null(maxY) && maxY != "") {
    plotDataChart$chart <- map(plotDataChart$chart, hc_yAxis, max = maxY, endOnTick = FALSE, startOnTick = FALSE)
  }
  
  
  if(!is.null(minX) && minX != "") {
    plotDataChart$chart <- map(plotDataChart$chart, hc_xAxis, min = minX, endOnTick = FALSE, startOnTick = FALSE)
  }
  
  if(!is.null(maxX) && maxX != "") {
    plotDataChart$chart <- map(plotDataChart$chart, hc_xAxis, max = maxX, endOnTick = FALSE, startOnTick = FALSE)
  }
  ...
  if(!is.null(nchart)) {
    ...
    if (plot_type %in% c("plotDisagLine_compare_hc")) {
      ...
      mod_subtitle <- paste(" ", tolower(modal_chart$dimension), " in ",
                            modal_chart$ncountry, 
                            modifier                   
      ) #subtitle DISAGG FOCUS
    }
    
    
    if (plot_type %in% c("plotSummaryBar_compare_hc")) {
      ...
      mod_title <- paste(
        modal_chart$indic_name#,modal_chart$indic_name2,
        #str_glue_data(
         # list(
          #  setting = val,
    ...
    hc <- modal_chart$chart[[1]] %>% 
      ...
      hc_exporting(
        ...
        filename = switch(
          ...
          plotSummaryBar_compare_hc = "summary_measures",
          "data"
        ),
        fallbacktoExportServer = FALSE,
        # needed to work in firefox and avoid the bug that otherwise would make the app crash
  ...
  highchart() %>% 
    ...
    hc_plotOptions(
      series = list(
        events = list(legendItemClick = JS("function(){ return false;}")),
        marker = list(
          symbol = "circle" # cambia il simbolo della legenda del grafico principale
        )
      )
    ) %>% 
    hc_legend(
