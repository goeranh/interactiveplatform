...
#******************************************************************************
# Compare inquality: category selector
#******************************************************************************
output$focus_category_compare <- renderUI({
  
  #label <- translate("compare_disag_select_indicator", input$lang)
  label <- "Select HESRi indicator type"        # TODO: get right translation !!
  
  choices <- .rdata[['full_categories']]$category
  
  selectInput(
    inputId = "focus_category_compare",
...
#******************************************************************************
# Compare inquality: health indicator selector
#******************************************************************************
...
# ************************************************************************
# ************************** INDICATORS METADATA SECTION *****************
# ************************************************************************


# SELECT CATEGORY OF INDICATORS
output$metadata_categories <- renderUI({
  
  categories = .rdata[['full_categories']]$category

  selectInput(
    inputId = "metadata_category", 
    label = "Select HESRi indicator type",
    choices = categories, 
...
# SELECT INDICATOR
output$metadata_indicators <- renderUI({
  
  if(is.null(input$metadata_category))
    return()
  
  cat_indicators <- select(filter(.rdata[['categories_indicators']], category == input$metadata_category), code)
  
  cat_indicators <- filter(unique(.rdata[['strata']][,.(indic, indic_name)]), indic %in% cat_indicators$code)
  indicators <- cat_indicators$indic[sort.list(cat_indicators$indic_name)]#ELE #cat_indicators$indic
  names(indicators) <- sort(cat_indicators$indic_name)
  
  selectInput(
    inputId = "metadata_indicator", 
...
# INDICATOR METADATA LIST
output$indicators_metadata <- renderUI({
  
  if(is.null(input$metadata_indicator)) return()
  
  indicatorsData = list_parse(filter(select(.rdata[['indicators_metadata']],
                                            name = title,
                                            description = `Indicator notes`,
                                            notes = `Country notes / Data availability`,
                                            code = code,
                                            icon = Icon_cat),
                                     code == input$metadata_indicator))
  
  if(length(indicatorsData) == 0) return()
  
  # the indicators data output list
  outputData = list()
  
  # setting the correct encoding based on file and filesystem
  # It doesn't make too much sense setting the encoding this way (Encoding(as.character(indicatorsData[[1]]$description))), many cases cause encoding="unknown"
  # encoding = Encoding(as.character(indicatorsData[[1]]$description))
  encoding = "UTF-8"
  
  for(indicator in indicatorsData) {

    outputData[[length(outputData) + 1]] = list(
      #tags$h2(HTML(iconv(indicator$name, from = encoding, to = "ASCII//TRANSLIT"))), 
      tags$p(HTML(iconv(indicator$description, from = encoding, to = "ASCII//TRANSLIT"))),
      tags$p(HTML(iconv(indicator$notes, from = encoding, to = "ASCII//TRANSLIT")))
    )
  }
  ...
  list(
    ...
    conditionalPanel( 
      ...
      selectInput(
        inputId = "focus_year_compare", 
        label = NULL, 
        choices = c(format(Sys.Date(), "%Y"):1980), 
        multiple = FALSE, 
...
output$focus_source_recent_year_compare <- renderUI({
  lang <- input$lang
  label_sources <- translate("sources", lang)
  label_years <- translate("compare_disag_select_year", lang)
  label_recent <- translate("recent", lang)
...
# 
#   # cat("\nserver_ui_creation - benchmark countries - All:", length(.rdata[["all_countries"]]), "- Selected for benchmark section:", length(.rdata[["benchmark_countries"]]), "\n")
# 
...
output$benchmarkWBgroup <- renderUI({
  ...
  # cat("\nserver_ui_creation - Income groups:", .rdata[["income_groups"]], "\n")
...
#    selected <- choices
#   
#  } else if(is.null(input$focus_country_compare) & !is.null(input$focus_country_explore)){

#    selected <- getCountryWHOregionIncome(input$focus_country_explore)$region
#  } else{
#    selected <- getCountryWHOregionIncome(input$focus_country_compare)$region
#  }
#  .rdata[['focus_who_regions']] <<- selected
