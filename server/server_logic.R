...
output$summary_plot_explore <- renderUI({ 
  ...
  if(!dropplot){ 
    ...
    # cat("\nserver_logic - summary_plot_explore - data:\n")
    # print(plotData)
...
output$disag_plot_compare <- renderUI({
  ...
  isolate({
    ...
    benchmark_countries <- .rdata[['benchmark_countries']]
  ...
  if(!dropplot){
    ...
    #if(is.null(input$benchmarkYears)) return()
    # print(c(focus_country_compare, benchmark_countries))
    if(!is.null(mostrecent_compare) && mostrecent_compare) benchmarkYears = 100
    
    anchordata<-getDisagData(indicator=focus_indicator_compare, 
                             stratifier=focus_dimension_compare,  # in hetkdb.R
                             countries=benchmark_countries,  #.rdata[["all_countries"]],
    ...
    someAnchorData <- nrow(plotData)>0
...
output$summary_plot_compare <- renderUI({
  ...
  # to let users know that there are situations where no data is available
  # dataMSG <- "If estimates are not shown for a selected combination of variables, then data are not available."
  dataMSG <- paste(translate("noestimates", input$lang), "If trends are not shown for a country, then historical data is not available.")
  
  if(is.null(input$xaxis_limitsmin4 )) return()
  if(is.null(input$xaxis_limitsmax4 )) return()
  axismin <- suppressWarnings(as.numeric(input$xaxis_limitsmin4))
  axismax <- suppressWarnings(as.numeric(input$xaxis_limitsmax4))
  isValid1 <- input$xaxis_limitsmin4 == "" | !is.na(axismin)
  isValid2 <- input$xaxis_limitsmax4 == "" | !is.na(axismax)
  longnames <- input$long_names2
  ...
  isolate({
   ...
   # if(is.null(input$focus_country_compare) || trimws(input$focus_country_compare) == "") return()
    ...
    focus_indicator_compare <- input$focus_indicator_compare  
    focus_data_source_compare <- input$focus_data_source_compare  
    #ELE focus_country_compare <- .rdata[["all_countries"]][1] #input$focus_country_compare
    focus_dimension_compare <- input$focus_dimension_compare 
    benchmarkYears  <-  input$benchmarkYears
    benchmark_countries <- .rdata[['benchmark_countries']]
  ...
  if(!dropplot){
    ...
    anchordata <- getInequalData(indicator=focus_indicator_compare,  
                                 stratifier=focus_dimension_compare, 
                                 countries= benchmark_countries, #ELE .rdata[["all_countries"]][1], #focus_country_compare, 
                                 mostrecent=TRUE,
                                 datasource=focus_data_source_compare,  
    ...
    regressionYears = c(1990:2030)
    regressionData <- getInequalData(indicator=focus_indicator_compare,  
                                     stratifier=focus_dimension_compare, 
                                     countries=benchmark_countries,  #.rdata[["all_countries"]][1], 
                                     years=regressionYears, 
                                     mostrecent=FALSE,
                                     datasource=focus_data_source_compare,  
                                     inequal_types=focus_inequal_type_compare)
    ...
    if(!is.null(benchmark_countries)){
      ...
      # data for the regression
      regressionDataOthers <- getInequalData(indicator=focus_indicator_compare,
                                             stratifier=focus_dimension_compare,
                                             countries=benchmark_countries,
                                             years=regressionYears, 
                                             mostrecent=FALSE,
      ...
      if(!is.null(regressionDataOthers) && nrow(regressionDataOthers)!=0){
        
        regressionData <- rbind(regressionData, regressionDataOthers)
      }
    }
    ...
    plotData <- filter(plotData, 
                       measure == focus_inequal_type_compare, !is.na(inequal))
    plotData <- plot_decimals_setup(plotData, type = "sum")
    
    regressionData <- filter(regressionData, 
                             measure == focus_inequal_type_compare, !is.na(inequal))
    regressionData <- plot_decimals_setup(regressionData, type = "sum")
    
    .rdata[['focus_plot_regression_summary_compare']] <<- regressionData
    ...
    # thetest <- nonNull_plotData  && someAnchorData && notAllNA_Anchor
    thetest <- nonNull_plotData  && someAnchorData
    ...
    if(!thetest) {
      ...
      if(length(isLogScale)>0 && isLogScale){
        ...
        # test for log scale and incorrect y params
        okLogMin <- input$xaxis_limitsmin4 == "" | axismin > 0
        okLogMax <- input$xaxis_limitsmax4 == "" | axismax > 0
  ...
  output$summary_plot_compare_insideHC <- renderUI({
    ...
    doPlotType <- get("plotSummaryBar_compare_hc")
    p <- doPlotType(plotData, regressionData)
