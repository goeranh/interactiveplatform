...
observeEvent(c(input$focus_year_compare, .trigger$focus_year_compare, input$lang), {
  ...
  if(!input$focus_year_compare%in%.rdata[['focus_year_compare']]){
    .rdata[['focus_year_compare']] <<- input$focus_year_compare
  }
  
  # ---- Resets the indicator and dimension lists
  
  reset_focus_indic_dim(yr = .rdata[['focus_year_compare']])
...
observeEvent(c(input$focus_category_compare, .trigger$focus_categ_compare, input$lang), {
  req(c(input$focus_category_compare, .trigger$focus_categ_compare))
  
  if(is.null(input$focus_category_compare) || trimws(input$focus_category_compare) == "") return()
  
  update_global_indicator("focus_category_compare", input$focus_category_compare)
  update_indicator_boxes("focus_category_compare")

  # update indicators select
...
observeEvent(c(input$focus_dimension_explore_dtl, .trigger$focus_dim_explore_plotdtl, input$lang), {
  ...
  if(length(indics)>0){
    ...
    updateSelectizeInput(session, "disag_plot_explore_dtl_sort",
                         choices = .rdata[['full_indicators']],
                         selected = input$focus_indicator_explore_plotdtl)
  }else{
...
observeEvent(c(.trigger$focus_WHO_info, input$lang), {
  ...
  if(is.null(input$focus_country_compare) & !is.null(input$focus_country_explore)){
    focus <- input$focus_country_explore
  } else{
    focus <- input$focus_country_compare
  }
  country_info<-getCountryWHOregionIncome(focus)
  
  choices <- .rdata[["income_groups"]]
  
  .rdata[['focus_income_group']] <<- choices
  ...
  updateSelectInput(session, "benchmarkWBgroup", selected = choices)
  ...
  updateSliderInput(session, "benchmarkYears", value = 5)
...
observeEvent(c(input$benchmarkWBgroup, input$benchmarkWHOregion, input$lang), {
  ...
  # Not interested anymore to remove the focus country
  # tmpCountries <- tmpCountries[!tmpCountries%in%.rdata[['focus_country']]] 
  ...
  if(length(tmpCountries)!=0){
    ...
    selected <- unique(tmpCountries)
  }
  
  # updateSelectInput(session, "benchmark_countries", choices = choices, selected = selected)
  
  .rdata[['benchmark_countries']] <<- tmpCountries
  .trigger$runplot_disag_compare <- !.trigger$runplot_disag_compare
  .trigger$runplot_summary_compare <- !.trigger$runplot_summary_compare
...
# Button for redirect in Compare tab
observeEvent(input$heat_plus_landing_button_to_compare, {
  updateTabsetPanel(session, "who_heat",
                    selected = "compare")
})